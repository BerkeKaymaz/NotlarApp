...
class Notlardao {
    ...
    @SuppressLint("Range")
    fun tumNotlar(vt: DBHelper):ArrayList<Notlar>{
        val db = vt.writableDatabase
        val notListe = ArrayList<Notlar>()
    ...
    fun notSil(vt:DBHelper,not_id:Int){
        val db = vt.writableDatabase
        db.delete("notlar","not_id=?", arrayOf(not_id.toString()))
        db.close()
    }

    fun notEkle(vt: DBHelper,ders_adi:String,not1:Int,not2:Int){
        val db = vt.writableDatabase
    ...
    fun notGuncelle(vt: DBHelper,not_id: Int,ders_adi:String,not1:Int,not2:Int){
        val db = vt.writableDatabase
        val values = ContentValues()
        values.put("ders_adi",ders_adi)
        values.put("not1",not1)
